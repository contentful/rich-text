...
> **Note on this revision:** the previous version of this file described a `yarn`-based workflow
> (`yarn build`, `yarn commit` via commitizen, `npm install` hoisting behavior) and Lerna-based publishing.
> The repo has since migrated to pnpm (`3eddc41`), GitHub Actions (`140b093`), and Nx Release (`0e40927`).
> This revision reflects the current `package.json`/CI state; the commands below are cited to their source.
...
- **Location:** `packages/<name>/src/__test__/*.test.ts(x)`, except `rich-text-types`, whose tests live in
  `packages/rich-text-types/__test__/` (package-root-level, not under `src/`) `[INFERRED — no documented
  reason found for the inconsistency]`.
- **Run all:** `pnpm test`.
...
- **Known gap:** `contentful-slatejs-adapter/src/slatejs-to-contentful-adapter.ts` has no corresponding
  test file — only the Contentful→Slate direction
  (`contentful-to-slatejs-adapter.test.ts`) is covered. `[NEEDS TEAM INPUT — confirm whether this is
...
- Publishing is fully automated via Nx Release in CI. On every merge to `master`:
  ...
  - Per-package `CHANGELOG.md` files are frozen at their last Lerna-generated state and are **not** updated
    ...
    [`0e40927` — `chore(ci): migrate release versioning from lerna to nx release`, `nx.json` → `release`]
...
| `packages/rich-text-plain-text-renderer/bin/` | `[POSSIBLY DEAD CONFIG]` — `tsconfig.json` here extends `../../../bin/tsconfig.json`, which no longer exists (removed when benchmark scripts were deleted in `8c320bd`). Not referenced by any script; last touched in 2018. Confirm before deleting. |
